...
TaskManager.defineTask(BACKGROUND_TX_TASK, async () => {
  try {
    ...
    const lastWalletName = mobileStorage.readJSON<string>(
      "last_wallet_name",
      walletNames[0],
    );
    ...
    // 3. Get current network and full config
    const storageConfig = mobileStorage.readJSON<{
      network: string;
      networks?: Record<string, any>;
    }>("config.json", { network: "sepolia" });
    const network = storageConfig.network;

    console.log(
      `[BackgroundService] Checking ${network} for ${address.slice(0, 6)}...`,
    );

    // 4. Configure Explorer API
    const { explorerAPI } = require("@wallet/explorer-api");
    
    // Support custom networks by reading from storage config first, then falling back to bundled
    ...
    // NOTE: This assumes Etherscan-compatible API. JSON-RPC (localhost) will fail here.
    ...
    const lastHash = mobileStorage.readJSON<string | null>(
      LAST_TX_HASH_KEY,
      null,
    );
    ...
    if (lastHash !== latestHash) {
      console.log(
        "[BackgroundService] New transaction detected! Triggering notification.",
      );

      // 7. Schedule Notification
      const fromAddr = latestTx.from || "";
      const type =
        fromAddr.toLowerCase() === address.toLowerCase() ? "Sent" : "Received";
      const symbol = latestTx.tokenSymbol || "ETH";

      // Format amount with max 6 decimals to prevent overflow
      let formattedAmount = "0";
      try {
        const val = BigInt(latestTx.value || "0");
        const eth = Number(val) / 1e18;
        formattedAmount = eth.toLocaleString(undefined, {
          maximumFractionDigits: 6,
        });
      } catch (e) {
        console.warn("[BackgroundService] Failed to format value:", e);
      ...
      // Update storage ONLY after successful notification
